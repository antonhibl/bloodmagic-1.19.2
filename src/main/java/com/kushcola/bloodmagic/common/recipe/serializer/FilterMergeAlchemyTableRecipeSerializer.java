		...
		implements RecipeSerializer<RECIPE>
{
	...
	public RECIPE fromJson(
		...
		JsonElement filterElem = GsonHelper.isArrayNode(json, Constants.JSON.FILTER)
				? GsonHelper.getAsJsonArray(json, Constants.JSON.FILTER)
				: GsonHelper.getAsJsonObject(json, Constants.JSON.FILTER);
		Ingredient filter = Ingredient.fromJson(filterElem);
		...
		if (json.has(Constants.JSON.INPUT) && GsonHelper.isArrayNode(json, Constants.JSON.INPUT)) {
			...
			for (JsonElement elem : mainArray) {
				if (inputList.size() >= RecipeFilterMergeAlchemyTable.MAX_INPUTS) break;
